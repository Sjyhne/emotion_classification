...
def get_actors(filepaths):
    ...
    for filepath in filepaths:
        actors.append(filepath.split("/")[-1][:3])
...
def get_datatype_paths(filepaths):
    ...
    for path in filepaths:
        if not get_shemo_label(path) == "N/A":
            actor = path.split("/")[-1][:3]
...
def get_shemo_label(filepath):
    filename = filepath.split("/")[-1]
    emotion_letter = filename[3]
    try:
...
def get_class_diversity(filepaths):
    ...
    for filepath in filepaths:
        label = get_shemo_label(filepath)
        if label != "N/A":
...
def get_data(datadir):
    ...
    print("ShEMO class diversity:", class_diversity)

    print("ShEMO file duration span:", utils.get_duration_span(filepaths))
